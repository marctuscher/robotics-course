...
import numpy as np
#%%
p = pathlib.Path('/home/niklas/git/uni/imitation-learning/bagfiles/demo_zero')
os.chdir(p)
files = os.listdir(p)
#%%
files
...
for fname in files:
    ...
    for _, msg, _ in bag.read_messages():
        ...
        a = np.concatenate([np.array(pos), np.array(vel)])
        if len(pos) == 7 and len(vel)==7:
            tmp.append(a)
        if i == 0:
            print(tmp)
            #print(vel)
...
len(demos[0])
for i in range(len(demos[0])):
    if demos[0][i].shape != (14,):
        print(demos[0][i])
#%%
...
import pickle
#%%
with open("/home/niklas/git/uni/imitation-learning/bagfiles/demo_zero/dataset.pkl", "wb") as f:
